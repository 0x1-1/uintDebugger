...
## Changelog
### Version 0.2.0
+ migrated the project from Qt4 + VS2010 to Qt 6.10.2 + CMake
+ renamed the active codebase and UI surface to uintDebugger
+ fixed multiple x64 startup and runtime crashes in the migrated build
+ refreshed the dark UI styling, about dialog, and dock/widget layout handling
+ centralized application version metadata in the top-level CMake project
+ added GitHub Releases based startup update checks and updater manifest support
+ replaced the legacy updater.ini flow with manifest driven file updates
...
+ added possibility to set uintDebugger also as wow64 jit debugger
...
+ added possibility to set uintDebugger as default just in time debugger
...
####Notes:
	- CrashHandler
		- if uintDebugger crashs a dumpfile will be written to the application folder.
	- Hotkey "Return"
...
- dependencies:
	- For developers:	You will need a QT Framework which has been compiled with /MT ( or /MTd) else you
						have a dependencie of the cruntime even if qtUintDebugger has been compiled without.
