...
clsProcessDump::clsProcessDump(HANDLE processHandle, DWORD processID, QString filePath, QWidget *pParent)
{
	...
	if(dumpFile != INVALID_HANDLE_VALUE) 
	{
		...
		if(MiniDumpWriteDump(processHandle, processID, dumpFile, dumpTyp, NULL, NULL, NULL))
			QMessageBox::information(pParent,"uintDebugger","Process dump was successfull!",QMessageBox::Ok,QMessageBox::Ok);
	
		CloseHandle(dumpFile); 
		return;
	}

	QMessageBox::critical(pParent,"uintDebugger","Process dump has failed!",QMessageBox::Ok,QMessageBox::Ok);
}
