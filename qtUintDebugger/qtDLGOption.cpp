...
qtDLGOption::qtDLGOption(QWidget *parent, Qt::WindowFlags flags)
...
//		MessageBoxW(NULL,L"ERROR, could not write the default jit!\r\nDo you have Admin rights?",L"uintDebugger",MB_OK);
	...
	QMessageBox::information(this, "uintDebugger", "Your settings have been saved!", QMessageBox::Ok, QMessageBox::Ok);
}
...
void qtDLGOption::OnLoad()
{
	...
	for(int i = 0; i < m_pMainWindow->coreDebugger->ExceptionHandler.size(); i++)
	{
		...
		tblCustomExceptions->setItem(tblCustomExceptions->rowCount() - 1,1,
			new QTableWidgetItem(QString::asprintf("%d",m_pMainWindow->coreDebugger->ExceptionHandler[i].dwAction)));
	}
	...
	if(!JITString.contains("uintDebugger.exe"))
	{
	...
	if(!WOW64JITString.contains("uintDebugger.exe"))
	{
...
void qtDLGOption::OnSetUintDebuggerDefault()
{
...
{
	QSettings registerExtension("HKEY_CURRENT_USER\\Software\\Classes\\uintDebugger.debugger.v1\\shell\\open\\command", QSettings::NativeFormat);
	registerExtension.setValue(".", "\"" + QApplication::applicationFilePath().replace("/", "\\") + "\" -f \"%1\"");
	registerExtension.sync();

	QSettings registerDefaultIcon("HKEY_CURRENT_USER\\Software\\Classes\\uintDebugger.debugger.v1\\DefaultIcon", QSettings::NativeFormat);
	...
	registerFileExtension.setValue(".", "uintDebugger.debugger.v1");
	...
	if(registerFileExtension.status() == QSettings::NoError && registerExtension.status() == QSettings::NoError && registerDefaultIcon.status() == QSettings::NoError)
		QMessageBox::information(this, "uintDebugger", "File extension registered successfully!", QMessageBox::Ok, QMessageBox::Ok);
}
...
void qtDLGOption::DisableNDBExtension()
{
	...
	QSettings registerFileIcon("HKEY_CURRENT_USER\\Software\\Classes\\uintDebugger.debugger.v1\\DefaultIcon", QSettings::NativeFormat);
	...
	if(registerFileExtension.status() == QSettings::NoError && registerExtension.status() == QSettings::NoError && registerFileIcon.status() == QSettings::NoError)
		QMessageBox::information(this, "uintDebugger", "File extension removed successfully!", QMessageBox::Ok, QMessageBox::Ok);
}
