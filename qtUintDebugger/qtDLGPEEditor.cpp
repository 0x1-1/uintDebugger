...
qtDLGPEEditor::qtDLGPEEditor(clsPEManager *PEManager,QWidget *parent, Qt::WindowFlags flags, int PID, QString FileName, DWORD64 imageBase)
	: QWidget(parent,flags)
{
	...
	{	
		...
		{
			QMessageBox::critical(this,"uintDebugger","Could not load File!",QMessageBox::Ok,QMessageBox::Ok);

			close();
		}
		this->setWindowTitle(QString("[uintDebugger] - PE Editor - File: %1").arg(m_currentFile));
...
void qtDLGPEEditor::InsertSections()
{
	...
	for(int i = 0; i < sections.size(); i++)
	{
		...
		sectionElement->setText(0,QString::fromLatin1((char*)sections.at(i).Name, sizeof(sections.at(i).Name)));
...
void qtDLGPEEditor::MenuCallback(QAction* pAction)
{
	if(QString().compare(pAction->text(),"Show Offset in disassembler") == 0)
	{
		...
		{
			qtDLGUintDebugger::GetInstance()->DisAsGUI->OnDisplayDisassembly(m_selectedOffset);
