...
void qtDLGRegisters::MenuCallback(QAction* pAction)
{
	...
	{
		if(!qtDLGUintDebugger::GetInstance()->coreDisAs->InsertNewDisassembly(m_pDebugger->GetCurrentProcessHandle(),tblRegView->item(m_iSelectedRow,1)->text().toULongLong(0,16)))
	...
	else if(pAction->text().compare("Send to StackView") == 0)
	{
		...
		if(bIsWOW64)
			qtDLGUintDebugger::GetInstance()->stackView->LoadStackView(tblRegView->item(m_iSelectedRow,1)->text().toULongLong(0,16), 4);
		else
			qtDLGUintDebugger::GetInstance()->stackView->LoadStackView(tblRegView->item(m_iSelectedRow,1)->text().toULongLong(0,16), 8);
#else
		qtDLGUintDebugger::GetInstance()->stackView->LoadStackView(tblRegView->item(m_iSelectedRow,1)->text().toULongLong(0,16),4);
#endif
	}
