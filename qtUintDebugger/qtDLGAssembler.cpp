...
qtDLGAssembler::qtDLGAssembler(QWidget *parent, Qt::WindowFlags flags, HANDLE processHandle, quint64 instructionOffset, QString currentInstruction, clsDisassembler *pCurrentDisassembler, bool is64Bit)
...
void qtDLGAssembler::InsertNewInstructions()
{
	...
	QFile tempOutput("uintDebugger.asm");
	...
	out << "org 0x" << Qt::hex << i.value().Offset << "\r\n";
	out << lineEdit->text();
	tempOutput.close();
    ...
    nasm.start("nasm.exe -o uintDebugger.bin uintDebugger.asm");
    if (!nasm.waitForStarted())
	{
		QMessageBox::critical(this, "uintDebugger", "Unable to launch assembler!", QMessageBox::Ok, QMessageBox::Ok);
		close();
		return;
	}
	...
	while(nasm.state() != QProcess::NotRunning)
	{
		...
		if(errorMessage.contains("uintDebugger.asm:3:"))
		{
			errorMessage.replace("uintDebugger.asm:3:","");
			QMessageBox::critical(this, "uintDebugger", errorMessage, QMessageBox::Ok, QMessageBox::Ok);
			lineEdit->clear();
			return;
		}
    }
	DeleteFile(L"uintDebugger.asm");

	HANDLE hFile = CreateFileW(L"uintDebugger.bin",GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,NULL,NULL);
	...
	{
		CloseHandle(hFile);
		DeleteFile(L"uintDebugger.bin");
		clsMemManager::CFree(pFileBuffer);
		QMessageBox::critical(this,"uintDebugger","no valid opcodes found!",QMessageBox::Ok,QMessageBox::Ok);
		lineEdit->clear();
		return;
	}
	CloseHandle(hFile);
