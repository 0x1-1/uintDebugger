...
qtDLGFunctions::qtDLGFunctions(qint32 processID, QWidget *parent, Qt::WindowFlags flags)
...
{
	if(m_pFunctionWorker != NULL)
	{
		m_pFunctionWorker->disconnect(this);
		delete m_pFunctionWorker;
		m_pFunctionWorker = NULL;
	}
...
{
	if(m_pFunctionWorker->functionList.count() <= 0)
	{
		SetPlaceholderRow(tblFunctions, QStringLiteral("No functions were discovered for this view."));
		return;
	}
