...
void qtDLGCallstack::OnDisplaySource(QTableWidgetItem *pItem)
{
	...
	qtDLGSourceViewer *pSourceView = qtDLGUintDebugger::GetInstance()->DisAsGUI->dlgSourceViewer;
	...
	{
		QMessageBox::critical(this, "uintDebugger", "Unable to open source file!", QMessageBox::Ok, QMessageBox::Ok);
	}
	...
	for(int i = 0; i < callstackDisplayData.count(); i++)
	{
		...
		if(callstackDisplayData.at(i).sourceLineNumber > 0 && callstackDisplayData.at(i).sourceFilePath.length() > 0)
		{
			...
			tblCallstack->setItem(tblCallstack->rowCount() - 1,5,
				new QTableWidgetItem(QString::asprintf("%d",callstackDisplayData.at(i).sourceLineNumber)));
...
{
	clsDebugger *pDebugger = qtDLGUintDebugger::GetInstance()->coreDebugger;
