...
qtDLGTrace::qtDLGTrace(QWidget *parent, Qt::WindowFlags flags)
...
void qtDLGTrace::OnShow(int delta)
{
	...
	while(printLine <= m_maxRows)
	{
		...
		{
			clsHelperClass::LoadSymbolForAddr(FuncName,ModName,currentTraceData.dwOffset,qtDLGUintDebugger::GetInstance()->coreDebugger->GetProcessHandleByPID(currentTraceData.PID));
