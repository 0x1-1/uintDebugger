...
void qtDLGUintDebugger::action_FileTerminateGUI()
{
...
void qtDLGUintDebugger::action_FileOpenNewFile()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_IsRestart = true;
...
void qtDLGUintDebugger::action_FileAttachTo()
{
	if(!coreDebugger->GetDebuggingState())
	{
...
void qtDLGUintDebugger::action_FileDetach()
{
	if(coreDebugger->GetDebuggingState())
	{
		if(!coreDebugger->DetachFromProcess())
			MessageBox(NULL,L"Failed to detach from Process!",L"uintDebugger",MB_OK);
		else
			UpdateStateBar(STATE_TERMINATE);
	}
}

void qtDLGUintDebugger::action_FileRestartAsAdmin()
{
	WCHAR fileName[MAX_PATH] = {0};
	if(GetModuleFileNameW(NULL, fileName, MAX_PATH) <= 0)
	{
		QMessageBox::critical(this, "uintDebugger", "Failed to locate the current executable.", QMessageBox::Ok, QMessageBox::Ok);
		return;
	}

	if((INT_PTR)ShellExecuteW(NULL, L"runas", fileName, NULL, NULL, SW_SHOW) <= 32)
	{
		QMessageBox::warning(this, "uintDebugger", "Administrator restart was cancelled or failed.", QMessageBox::Ok, QMessageBox::Ok);
		return;
	}
...
void qtDLGUintDebugger::action_FileLoad()
{
	if(coreDebugger->GetDebuggingState())
	{
...
void qtDLGUintDebugger::action_FileSave()
{
	if(coreDebugger->GetDebuggingState())
		clsProjectFile(true);
	else
		QMessageBox::warning(this, "uintDebugger", "It seems there is currently nothing to be saved!", QMessageBox::Ok, QMessageBox::Ok);
}

void qtDLGUintDebugger::action_DebugStart()
{
	if(!coreDebugger->GetDebuggingState())
	{
		CleanGUI();
...
void qtDLGUintDebugger::action_DebugAttachStart(int iPID,QString FilePath)
{
	if(!coreDebugger->GetDebuggingState())
	{
		ClearDebugData(true);
...
void qtDLGUintDebugger::action_DebugStop()
{
	if(coreDebugger->GetDebuggingState())
		coreDebugger->StopDebuggingAll();
}

void qtDLGUintDebugger::action_DebugRestart()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_IsRestart = true;
...
void qtDLGUintDebugger::action_DebugSuspend()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_DebugStepIn()
{
	if(coreDebugger->GetDebuggingState())
	{
		qtDLGTrace::clearTraceData();
...
void qtDLGUintDebugger::action_DebugStepOver()
{
...
void qtDLGUintDebugger::action_DebugStepOut()
{
	if(!coreDebugger->GetDebuggingState()) return;
	qtDLGTrace::clearTraceData();
...
void qtDLGUintDebugger::action_DebugRunToUserCode()
{
	if(!coreDebugger->GetDebuggingState()) return;
	qtDLGTrace::clearTraceData();
...
void qtDLGUintDebugger::action_OptionsAbout()
{
	qtDLGAbout DLGAbout(this,Qt::Window);
	DLGAbout.exec();
}

void qtDLGUintDebugger::action_OptionsOptions()
{
	qtDLGOption newOption(this,Qt::Window);
	newOption.exec();
}

void qtDLGUintDebugger::action_OptionsUpdate()
{
	UUpdateWidget::OpenAndCheckForUpdates(this);
}

void qtDLGUintDebugger::action_WindowDetailInformation()
{
	dlgDetInfo->show();
}

void qtDLGUintDebugger::action_WindowBreakpointManager()
{
	dlgBPManager->show();
}

void qtDLGUintDebugger::action_WindowPatches()
{
	dlgPatchManager->show();
}

void qtDLGUintDebugger::action_WindowShowMemory()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowHeap()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowStrings()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowDebugOutput()
{
	dlgDbgStr->show();
}

void qtDLGUintDebugger::action_WindowShowHandles()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowWindows()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowPEEditor()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowPrivileges()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_DebugTraceShow()
{
	dlgTraceWindow->show();
}

void qtDLGUintDebugger::action_DebugTraceStart()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_DebugTraceStop()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
...
void qtDLGUintDebugger::action_WindowShowFunctions()
{
	if(coreDebugger->GetDebuggingState())
	{
		m_iMenuProcessID = -1;
