	...
	m_processCountEntry(0)
{
	...
	SetPlaceholderRow(tblMemoryView, QStringLiteral("Loading memory map..."));
	QTimer::singleShot(0, this, SLOT(DisplayMemory()));
}
...
void qtDLGMemoryView::DisplayMemory()
{
	...
	if(tblMemoryView->rowCount() <= 0)
		SetPlaceholderRow(tblMemoryView, QStringLiteral("No memory regions available for the selected process."));
}
...
void qtDLGMemoryView::SetPageProctection(DWORD protectionFlag)
{
	...
	{
		QMessageBox::critical(this,"uintDebugger","Failed to change the Page protection!",QMessageBox::Ok,QMessageBox::Ok);
	}
	else
		DisplayMemory();
