...
void qtDLGLogView::MenuCallback(QAction* pAction)
{
	...
	else if(QString().compare(pAction->text(),"Write Log to File") == 0)
	{
		...
		QMessageBox::information(this,"uintDebugger","The log file has been written to disk!",QMessageBox::Ok,QMessageBox::Ok);
	}
...
void qtDLGLogView::OnLog(QString logString)
{
	...
	tblLogBox->setItem(0, 0, new QTableWidgetItem(QString::asprintf("[%i:%i:%i]",timeInfo.tm_hour,timeInfo.tm_min,timeInfo.tm_sec)));
