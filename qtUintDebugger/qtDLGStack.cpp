...
qtDLGStack::qtDLGStack(QWidget *parent)
	: QDockWidget(parent),
	m_pCoreDebugger(qtDLGUintDebugger::GetInstance()->coreDebugger)
{
	setupUi(this);
...
{	
	if(pObject == tblStack)
	{
		if(event->type() == QEvent::Wheel)
		{
			...
			OnStackScroll(pWheel->angleDelta().y() * -1);
			return true;
		}
...
void qtDLGStack::MenuCallback(QAction* pAction)
{
	...
	{
		qtDLGUintDebugger::GetInstance()->DisAsGUI->OnDisplayDisassembly(tblStack->item(m_selectedRow,1)->text().toULongLong(0,16));
	}
