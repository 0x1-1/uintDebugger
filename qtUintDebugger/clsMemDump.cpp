...
clsMemDump::clsMemDump( HANDLE hProc, PTCHAR FileBaseName, DWORD64 BaseOffset, DWORD Size, QWidget *pParent)
{
	...
	{
		QMessageBox::critical(pParent,"uintDebugger","Failed to read Memory!",QMessageBox::Ok,QMessageBox::Ok);
		free(pBuffer);
		return;
	}
	...
	if(wcslen(FileBaseName) <= 0)
		wsprintf(FileName,L"uintDebuggerDUMP_%016I64X-%016I64X_%08X.bin",BaseOffset,BaseOffset + Size,Size);
	else
	...
	{
		QMessageBox::critical(pParent,"uintDebugger","Failed to create File!",QMessageBox::Ok,QMessageBox::Ok);
		free(pBuffer);
		return;
	}

	if(!WriteFile(hFile,pBuffer,Size,&BytesWrote,NULL))
		QMessageBox::critical(pParent,"uintDebugger","Failed to write to File!",QMessageBox::Ok,QMessageBox::Ok);

	free(pBuffer);
	CloseHandle(hFile);

	QMessageBox::information(pParent,"uintDebugger","Memory Dump finished!",QMessageBox::Ok,QMessageBox::Ok);
}
