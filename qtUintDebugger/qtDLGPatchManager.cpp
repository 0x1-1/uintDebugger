...
qtDLGPatchManager::qtDLGPatchManager(QWidget *parent, Qt::WindowFlags flags)
...
void qtDLGPatchManager::MenuCallback(QAction* pAction)
{
	if(QString().compare(pAction->text(),"(Un)Do Patch") == 0)
	{
		...
		qtDLGUintDebugger::GetInstance()->coreDisAs->InsertNewDisassembly(qtDLGUintDebugger::GetInstance()->coreDebugger->GetCurrentProcessHandle(),
			tblPatches->item(m_iSelectedRow,1)->text().toULongLong(0,16),
			true);
...
bool qtDLGPatchManager::AddNewPatch(int PID, HANDLE hProc, quint64 Offset, int PatchSize, LPVOID newData, bool reWrite)
{
	...
	newPatch.processModule = qtDLGUintDebugger::GetInstance()->PEManager->getFilenameFromPID(PID);
	...
	qtDLGUintDebugger::GetInstance()->coreDisAs->InsertNewDisassembly(qtDLGUintDebugger::GetInstance()->coreDebugger->GetCurrentProcessHandle(), Offset, true);
...
{	
	...
	for(QList<PatchData>::iterator i = m_patches.begin(); i != m_patches.end(); ++i)
	{
		...
		{
			strTemp.append(QString::asprintf("%02X ",*(LPBYTE)((DWORD)i->orgData + a)));
		}
		...
		for(int a = 0; a < i->PatchSize; a++)
		{
			strTemp.append(QString::asprintf("%02X ",*(LPBYTE)((DWORD)i->newData + a)));
		}
...
{
	QString currentPModule = qtDLGUintDebugger::GetInstance()->PEManager->getFilenameFromPID(newPID);
	for(QList<PatchData>::iterator i = m_patches.begin(); i != m_patches.end(); ++i)
	{
