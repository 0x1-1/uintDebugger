...
qtDLGHandleView::qtDLGHandleView(QWidget *parent, Qt::WindowFlags flags,qint32 processID)
	...
	m_processCountEntry(0)
{
	...
	SetPlaceholderRow(tblHandleView, QStringLiteral("Loading handles..."));
	QTimer::singleShot(0, this, SLOT(OnDisplayHandles()));
}
...
void qtDLGHandleView::InsertDataIntoTable(DWORD dwPID,DWORD dwHandle,PTCHAR ptType,PTCHAR ptName)
{
	...
	tblHandleView->setItem(tblHandleView->rowCount() - 1,1,
		new QTableWidgetItem(QString::asprintf("%08X",dwHandle)));
...
void qtDLGHandleView::OnDisplayHandles()
{
	...
	for(int i = m_processCountEntry; i < m_processCountEnd;i++)
	{
		...
		{
			MessageBox(NULL,L"NtQuerySystemInformation failed!",L"uintDebugger",MB_OK);
			close();
		}
		...
		for (DWORD iCount = 0; iCount < handleInfo->HandleCount; iCount++)
		{
			...
			{
				InsertDataIntoTable(dwPID,(DWORD)handle.Handle,objectTypeInfo->Name.Buffer,(PTCHAR)L"Did not get name");
				free(objectTypeInfo);
				CloseHandle(dupHandle);
				continue;
			}
			...
			if (!NT_SUCCESS(clsAPIImport::pNtQueryObject(dupHandle,ObjectNameInformation,objectNameInfo,0x1000,&returnLength)))
			{
				...
				{
					InsertDataIntoTable(dwPID,(DWORD)handle.Handle,objectTypeInfo->Name.Buffer,(PTCHAR)L"Could not get name");
					free(objectTypeInfo);
					free(objectNameInfo);
					CloseHandle(dupHandle);
					continue;
			...
			if (objectName.Length)
				...
				InsertDataIntoTable(dwPID,(DWORD)handle.Handle,objectTypeInfo->Name.Buffer,(PTCHAR)L"unnamed");
	...
	if(tblHandleView->rowCount() <= 0)
		SetPlaceholderRow(tblHandleView, QStringLiteral("No handles available for the selected process."));
}
