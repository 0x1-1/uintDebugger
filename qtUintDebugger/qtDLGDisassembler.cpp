	...
	: QWidget(parent)
{
	...
	debuggerDisAsColor	= qtDLGUintDebugger::GetInstance()->disasColor;
	coreDisAs				= qtDLGUintDebugger::GetInstance()->coreDisAs;
	...
	if(coreDisAs->SectionDisAs.count() > 0 && dwEIP != 0)
	{
		...
		while(lineCount <= m_maxRows)
		{
			...
			if(clsBreakpointManager::IsOffsetAnBP(disassemblyDataCurrent.value().Offset))
				tblDisAs->item(lineCount,0)->setForeground(QColor(debuggerDisAsColor->colorBP));
			...
			if(itemStyle & COLOR_CALLS)
				tblDisAs->item(lineCount,2)->setForeground(QColor(debuggerDisAsColor->colorCall));
			else if(itemStyle & COLOR_JUMP)
				tblDisAs->item(lineCount,2)->setForeground(QColor(debuggerDisAsColor->colorJump));
			else if(itemStyle & COLOR_MOVE)
				tblDisAs->item(lineCount,2)->setForeground(QColor(debuggerDisAsColor->colorMove));
			else if(itemStyle & COLOR_STACK)
				tblDisAs->item(lineCount,2)->setForeground(QColor(debuggerDisAsColor->colorStack));
			else if(itemStyle & COLOR_MATH)
				tblDisAs->item(lineCount,2)->setForeground(QColor(debuggerDisAsColor->colorMath));
...
{	
	if(pObject == tblDisAs)
	{
		if(event->type() == QEvent::Wheel)
		{
			...
			OnDisAsScroll(pWheel->angleDelta().y() * -1);
			return true;
		}
...
void qtDLGDisassembler::CustomDisassemblerMenuCallback(QAction* pAction)
{
	...
	{
		QString commentString = QInputDialog::getText(this, "uintDebugger", "Comment:");
	...
	else if(pAction->text().compare("Trace to this") == 0)
	{
		...
		qtDLGUintDebugger *pMainWindow = qtDLGUintDebugger::GetInstance();
	...
	else if(pAction->text().compare("Show Source") == 0)
	{
		...
		if(dlgSourceViewer->IsSourceAvailable)
			...
			QMessageBox::information(this,"uintDebugger","Sorry, there is no source available!",QMessageBox::Ok,QMessageBox::Ok);
	}
...
void qtDLGDisassembler::OnF2BreakPointPlace()
{
	...
	if(clsBreakpointManager::BreakpointInsert(SOFTWARE_BP, BP_EXEC, -1, dwSelectedVA, 1, BP_KEEP))
		currentSelectedItems.value(0)->setForeground(QColor(debuggerDisAsColor->colorBP));
	else
	{// exists
...
void qtDLGDisassembler::OnF4ExecuteTo()
{
	...
	if(dwSelectedVA > 0 && clsBreakpointManager::BreakpointInsert(SOFTWARE_BP, BP_EXEC, -1, dwSelectedVA, 1, BP_STEPOVER))
	{
		...
		qtDLGUintDebugger::GetInstance()->UpdateStateBar(STATE_RUN);
	}
}
