...
qtDLGWindowView::qtDLGWindowView(QWidget *parent, Qt::WindowFlags flags,qint32 processID)
...
void qtDLGWindowView::EnumWindow()
{
	...
	if(tblWindowView->rowCount() <= 0)
		SetPlaceholderRow(tblWindowView, QStringLiteral("No top-level windows were found for the selected process."));
}
...
bool CALLBACK qtDLGWindowView::EnumWindowCallBack(HWND hWnd,LPARAM lParam)
{
	...
	if(dwHwPID == processID)
	{
		...
		pThis->tblWindowView->setItem(pThis->tblWindowView->rowCount() - 1,2,
			new QTableWidgetItem(QString(IsWindowVisible(hWnd) ? "TRUE" : "FALSE")));
