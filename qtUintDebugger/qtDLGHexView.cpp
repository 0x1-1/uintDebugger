...
qtDLGHexView::qtDLGHexView(QWidget *parent, Qt::WindowFlags flags,unsigned long dwPID, unsigned long long StartOffset,unsigned long long Size)
	...
	m_isFinished(false)
{
	...
	this->setWindowTitle(QString("[ uintDebugger ] - Show Memory - PID - %1 - From: %2 - To: %3").arg(dwPID, 8, 16, QChar('0')).arg(StartOffset, 8, 16, QChar('0')).arg(StartOffset + Size, 8, 16, QChar('0')));
...
{
	if(m_pHexDataWorker != NULL)
	{
		m_pHexDataWorker->disconnect(this);
		delete m_pHexDataWorker;
		m_pHexDataWorker = NULL;
	}
}

void qtDLGHexView::DisplayData()
{
	if(m_pHexDataWorker->dataList.count() <= 0)
	{
		SetPlaceholderRow(tblHexView, QStringLiteral("No readable memory data was returned for this range."));
		return;
	}
