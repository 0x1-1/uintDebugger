...
qtDLGBookmark::qtDLGBookmark(QWidget *parent, Qt::WindowFlags flags) :
	QWidget(parent, flags)
{
	...
	connect(new QShortcut(QKeySequence("F5"), this), &QShortcut::activated, this, &qtDLGBookmark::UpdateDisplay);
	connect(new QShortcut(Qt::Key_Escape,this),&QShortcut::activated,this,&qtDLGBookmark::close);
	connect(new QShortcut(QKeySequence(QKeySequence::Delete), this), &QShortcut::activated, this, &qtDLGBookmark::RemoveSelectedBookmark);
	connect(tblBookmark,&QTableWidget::itemDoubleClicked,this,&qtDLGBookmark::SendToDisassembler);
	connect(tblBookmark,&QTableWidget::cellChanged,this,&qtDLGBookmark::CellDataChanged);
}
...
{
	if(qtDLGUintDebugger::GetInstance()->coreDisAs->SectionDisAs.contains(bookmarkOffset))
	{
		qtDLGUintDebugger::GetInstance()->coreDisAs->SectionDisAs.clear();
...
bool qtDLGBookmark::BookmarkAdd(int processID, quint64 bookmarkOffset, QString bookmarkComment)
{
	...
	newBookmark.bookmarkProcessModule = qtDLGUintDebugger::GetInstance()->PEManager->getFilenameFromPID(processID);
...
{
	QString processModule = qtDLGUintDebugger::GetInstance()->PEManager->getFilenameFromPID(processID);
