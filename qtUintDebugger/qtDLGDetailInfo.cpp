...
qtDLGDetailInfo::qtDLGDetailInfo(QWidget *parent, Qt::WindowFlags flags)
...
void qtDLGDetailInfo::PIDMenuCallback(QAction* pAction)
{
	...
	else if(QString().compare(pAction->text(),"Show PBI/PEB") == 0)
	{
		...
		HANDLE selectedProcessHandle = qtDLGUintDebugger::GetInstance()->coreDebugger->GetProcessHandleByPID(selectedPID);
...
void qtDLGDetailInfo::MenuCallback(QAction* pAction)
{
	...
	else if(QString().compare(pAction->text(),"Suspend") == 0)
	{
		...
		{
			QMessageBox::critical(this,"uintDebugger","Have not been able to open this Thread!",QMessageBox::Ok,QMessageBox::Ok);
			return;
		}
		...
		if(SuspendThread(hThread) != -1)
			...
			QMessageBox::critical(this,"uintDebugger","Have not been able to suspend this Thread!",QMessageBox::Ok,QMessageBox::Ok);
	...
	else if(QString().compare(pAction->text(),"Resume") == 0)
	{
		...
		if(ResumeThread(hThread) != -1)
			...
			QMessageBox::critical(this,"uintDebugger","Have not been able to resume this Thread!",QMessageBox::Ok,QMessageBox::Ok);
	...
	else if(QString().compare(pAction->text(),"Show TBI/TEB") == 0)
	{
		...
		qtDLGTIBView *newTIBView = new qtDLGTIBView(qtDLGUintDebugger::GetInstance()->coreDebugger->GetProcessHandleByPID(procID),hThread,this,Qt::Window);
		newTIBView->show();
...
void qtDLGDetailInfo::OnException(QString functionName, QString moduleName, quint64 exceptionOffset, quint64 exceptionCode, DWORD processID, DWORD threadID)
{
	...
	tblExceptions->setItem(tblExceptions->rowCount() - 1,2,
		new QTableWidgetItem(QString::asprintf("%08X / %08X",processID,threadID)));
...
bool qtDLGDetailInfo::SetThreadPriorityByTid(DWORD threadID, int threadPrio)
{
	...
	if(!(bSuccess = SetThreadPriority(hThread,threadPrio)))
		QMessageBox::critical(this,"uintDebugger","Could not set the thread priority!",QMessageBox::Ok,QMessageBox::Ok);
	CloseHandle(hThread);
	return bSuccess;
}
